...
if PERSONAL_WORKSPACE_MODE:
    authentication_middleware_index = MIDDLEWARE.index(
        "django.contrib.auth.middleware.AuthenticationMiddleware"
    )
    MIDDLEWARE.insert(
        authentication_middleware_index + 1,
        "plane.middleware.personal_workspace.PersonalWorkspaceMiddleware",
    )
...
CELERY_TASK_EAGER_PROPAGATES = os.environ.get("CELERY_TASK_EAGER_PROPAGATES", "0") == "1"
